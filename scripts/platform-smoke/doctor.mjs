...
function parseLeaseId(output) {
  return output.match(/\bleased\s+(\S+)/)?.[1] ?? output.match(/\blease=(\S+)/)?.[1] ?? null;
}
...
function disposableWindowsSshProbe(cbox, config) {
  ...
  try {
    if (!warm.ok)
      return {
        ok: false,
        message: `disposable Windows warmup failed: ${(warm.stderr || warm.stdout).slice(-500)}`,
      };
    const probeCommand =
      "powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -Command 'Get-Command node,npm,git,tar -ErrorAction Stop | Out-Null; node --version; npm --version; git --version; tar --version | Select-Object -First 1; whoami'";
    const run = crabbox(
      cbox,
    ...
    if (!run.ok)
      return {
        ok: false,
        message: `disposable Windows probe failed: ${(run.stderr || run.stdout).slice(-500)}`,
      };
    const lines = run.stdout.trim().split(/\r?\n/).slice(-5);
    if (!/^v\d+\./.test(lines[0] ?? ""))
      return {
        ok: false,
        message: `disposable Windows node probe missing or invalid: ${lines.join(" | ")}`,
      };
    if (!/^\d+\.\d+\./.test(lines[1] ?? ""))
      return {
        ok: false,
        message: `disposable Windows npm probe missing or invalid: ${lines.join(" | ")}`,
      };
    if (!/^git version/i.test(lines[2] ?? ""))
      return {
        ok: false,
        message: `disposable Windows git probe missing or invalid: ${lines.join(" | ")}`,
      };
    if (!/tar/i.test(lines[3] ?? ""))
      return {
        ok: false,
        message: `disposable Windows tar probe missing or invalid: ${lines.join(" | ")}`,
      };
    if (!(lines[4] ?? "").trim())
      return {
        ok: false,
        message: `disposable Windows whoami probe missing: ${lines.join(" | ")}`,
      };
...
function parseVersion(version) {
  const match = /\d+(?:\.\d+){0,2}/.exec(String(version));
...
export async function runDoctor(config, options = {}) {
  ...
  const authEnv = (
    process.env.PLATFORM_SMOKE_AUTH_ENV
  ...
  if ((config?.requiredSuites ?? []).includes("goal-runtime-smoke")) {
    ...
    if (presentAuth.length > 0)
      ok(
        `auth env present: ${presentAuth.map((name) => `${name}=(present, redacted)`).join(", ")}`,
      );
    else
      fail(
        `no model auth env present; set one of ${authEnv.join(", ")} or PLATFORM_SMOKE_AUTH_ENV`,
        failures,
      );
  }
  ...
  if (cboxPath) {
    ...
    checkCrabboxProvider(
      cbox,
      buildTargetBaseArgs("ubuntu", config),
  ...
  const sshProbe = shell(
    `ssh -o BatchMode=yes -o ConnectTimeout=5 -o StrictHostKeyChecking=no ${sshUser}@${sshHost} 'node --version && npm --version && git --version'`,
  );
  ...
  if ((config?.requiredTargets ?? []).includes("windows-native")) {
    ...
    if (!hasCommand("prlctl")) {
      ...
      if (!list) {
        ...
        checkCrabboxProvider(
          cbox,
          buildTargetBaseArgs("windows-native", {
            ...config,
            windowsParallels: { ...windows, sourceVm: vmName, snapshot, user, workRoot },
          }),
          "windows parallels",
          failures,
        );
        const ipLine = shell(
          `prlctl list -f --no-header "${vmName.replace(/"/g, '\\"')}" 2>/dev/null`,
        );
        if (!ipLine) {
          ...
          if (ip && ip !== "-") {
            ...
            if (portCheck?.includes("open")) ok(`SSH open on ${ip}:22`);
              ...
              fail(
                `SSH not open on ${ip}:22 — enable OpenSSH Server in the Windows template VM`,
                failures,
              );
          } else if (skipWindowsDisposableProbe) {
            warn(
              `template ${vmName} has no IP; skipping disposable Windows clone probe because a full target run is expected to validate SSH/tools`,
            );
          } else if (cboxPath && snapshotMatch && snapshotPowerOff) {
            ok(
              `template ${vmName} has no IP; verifying Windows SSH/tools through a disposable Crabbox clone`,
            );
            ...
            fail(
              `Windows SSH probe could not run because ${vmName} has no IP and no verified poweroff snapshot was available`,
              failures,
            );
          }
        }
